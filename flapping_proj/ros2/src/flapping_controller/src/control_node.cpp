...
class ControlNode : public rclcpp::Node
{
	...
	private:
		...
		void motor_torque_output (){
			...
			// double external_gear_ratio = this->get_parameter("external_gear_ratio").as_double();
			...
			for (auto & joint : _jointTorqueControlMap) { //potential race condition? lmao
				...
				double effective_gear_ratio = 20; //motor_gear_ratio * external_gear_ratio * reduction_efficiency;
