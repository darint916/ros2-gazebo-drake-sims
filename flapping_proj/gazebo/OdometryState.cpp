...
#include <pose_v.pb.h> //either this or the <ignition/msgs/pose_v.pb.h> 
...
{
    this->data->model = igz::Model(_entity);
...
void OdometryState::PostUpdate(const igz::UpdateInfo &_info, const igz::EntityComponentManager &_ecm)
{
    ...
    //iterate through joint map with key and value
    for(auto const& joint : this->dataPtr->jointMap){
        ...
        double jointAngle = jointAnglePtr->Data(); //Radians for revolute joint
